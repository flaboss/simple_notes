...
# function to define the storage
...
class NoteManager:
    ...
    def _sync_from_remote():
        ...
        try:
            ...
            if response.status_code == 200 and response.json():
                ...
                current_store = get_actual_store()
                for note_id, note_data in remote_notes.items():
                    current_store.put(note_id, **note_data)
        except Exception as e:
    ...
    def get_all():
        try:
            ...
            notes = []
            current_store = get_actual_store()
            for key in current_store.keys():
                note = current_store.get(key)
                note["id"] = key
                notes.append(note)
    ...
    def get_one(note_id):
        current_store = get_actual_store()
        if current_store.exists(note_id):
            return current_store.get(note_id)
        return None
    ...
    def save(title, content):
        try:
            ...
            get_actual_store().put(note_id, **data)
